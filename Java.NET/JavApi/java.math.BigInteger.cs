...
namespace biz.ritter.javapi.math
{
    ...
    ///</summary>
    [Serializable]
    public class BigInteger : java.lang.Number, java.lang.Comparable<BigInteger>,
            java.io.Serializable {

        /// This is the serialVersionUID used by the sun implementation. */
        private static readonly long serialVersionUID = -8287574255936472291L;
        ...
        ///
        [NonSerialized]
        protected internal int [] digits;

        /// The length of this in measured in ints. Can be less than digits.length(). */
        [NonSerialized]
        protected internal int numberLength;
        ...
        /// The {@code BigInteger} constant -1. */
        protected internal static readonly BigInteger MINUS_ONE = new BigInteger(-1, 1);
        ...
        protected internal static readonly BigInteger[] SMALL_VALUES = { ZERO, ONE, new BigInteger(1, 2),
        ...
        protected internal static readonly BigInteger[] TWO_POWS;
        ...
        public BigInteger(int numBits, java.util.Random rnd) {
            if (numBits < 0) {
                ...
                throw new java.lang.IllegalArgumentException("numBits must be non-negative"); //$NON-NLS-1$
            }
            if (numBits == 0) {
                sign = 0;
                numberLength = 1;
                ...
                digits[numberLength - 1] = java.dotnet.lang.Operator.shiftRightUnsignet(digits[numberLength - 1],(-numBits) & 31);
                cutOffLeadingZeroes();
            }
        }
        ...
        public BigInteger(int bitLength, int certainty, java.util.Random rnd) {
            if (bitLength < 2) {
                // math.1C=bitLength < 2
                throw new java.lang.ArithmeticException("bitLength < 2"); //$NON-NLS-1$
            }
        ...
        public BigInteger(String val, int radix) {
            ...
            if ((radix < java.lang.Character.MIN_RADIX) || (radix > java.lang.Character.MAX_RADIX)) {
                ...
                throw new java.lang.NumberFormatException("Radix out of range"); //$NON-NLS-1$
            }
            if (val.length() == 0) {
                // math.12=Zero length BigInteger
                throw new java.lang.NumberFormatException("Zero length BigInteger"); //$NON-NLS-1$
            }
        ...
        public BigInteger(int signum, byte[] magnitude) {
            ...
            if ((signum < -1) || (signum > 1)) {
                ...
                throw new java.lang.NumberFormatException("Invalid signum value"); //$NON-NLS-1$
            }
            if (signum == 0) {
                foreach (byte element in magnitude) {
                    if (element != 0) {
                        // math.14=signum-magnitude mismatch
                        throw new java.lang.NumberFormatException("signum-magnitude mismatch"); //$NON-NLS-1$
                    }
                }
            }
        ...
        ///</summary>
        protected internal BigInteger(int sign, int value) {
            this.sign = sign;
            numberLength = 1;
        ...
        ///</summary>
        protected internal BigInteger(int sign, int numberLength, int[] digits) {
            this.sign = sign;
        ...
        ///</summary>
        protected internal BigInteger(int sign, long val) {
        ...
        ///</summary>
        protected internal BigInteger(int signum, int[] digits) {
            if (digits.Length == 0) {
                sign = 0;
                numberLength = 1;
        ...
        private static void setFromString(BigInteger bi, String val, int radix) {
            ...
            for (int substrStart = startChar; substrStart < endChar; substrStart = substrEnd, substrEnd = substrStart
                    + charsPerInt) {
                int bigRadixDigit = java.lang.Integer.parseInt(val.substring(substrStart,
                        substrEnd), radix);
        ...
        public bool testBit(int n) {
            ...
            if (n < 0) {
                ...
                throw new java.lang.ArithmeticException("Negative bit address"); //$NON-NLS-1$
            }
            int intCount = n >> 5;
        ...
        protected internal bool equalsArrays(int[] b) {
            int i;
        ...
        public BigInteger pow(int exp) {
            if (exp < 0) {
                ...
                throw new java.lang.ArithmeticException("Negative exponent"); //$NON-NLS-1$
            }
            if (exp == 0) {
                return ONE;
        ...
        public virtual BigInteger[] divideAndRemainder(BigInteger divisor) {
            ...
            if (divisorSign == 0) {
                ...
                throw new java.lang.ArithmeticException("BigInteger divide by zero"); //$NON-NLS-1$
            }
        ...
        public virtual BigInteger modInverse(BigInteger m) {
            if (m.sign <= 0) {
                ...
                throw new java.lang.ArithmeticException("BigInteger: modulus not positive"); //$NON-NLS-1$
            }
            // If both are even, no inverse exists
            if (!(testBit(0) || m.testBit(0))) {
                // math.19=BigInteger not invertible.
                throw new java.lang.ArithmeticException("BigInteger not invertible."); //$NON-NLS-1$
            }
            if (m.isOne()) {
                return ZERO;
            }
        ...
        public BigInteger modPow(BigInteger exponent, BigInteger m) {
            if (m.sign <= 0) {
                // math.18=BigInteger: modulus not positive
                throw new java.lang.ArithmeticException("modulus not positive"); //$NON-NLS-1$
            }
            BigInteger baseJ = this;

            if (m.isOne() | (exponent.sign > 0 & baseJ.sign == 0)) {
            ...
            BigInteger res = (m.testBit(0)) ? Division.oddModPow(baseJ.abs(),
                    exponent, m) : Division.evenModPow(baseJ.abs(), exponent, m);
            if ((baseJ.sign < 0) && exponent.testBit(0)) {
        ...
        public virtual BigInteger nextProbablePrime() {
            if (sign < 0) {
                // math.1A=start < 0: {0}
                throw new java.lang.ArithmeticException("start < 0: "+ this.ToString()); //$NON-NLS-1$
            }
        ...
        protected internal void cutOffLeadingZeroes() {
        ...
        protected internal virtual bool isOne() {
        ...
        ///
        protected internal BigInteger copy() {
            int[] copyDigits = new int[numberLength];
            java.lang.SystemJ.arraycopy(digits, 0, copyDigits, 0, numberLength);
...
*/
        protected internal virtual void unCache() {
            firstNonzeroDigit = -2;
        }

        protected internal static BigInteger getPowerOfTwo(int exp) {
